...
func _process(delta):
	if is_attacking:
		...
		position.x += speed * delta  # default movement
...
func _on_detector_area_entered(area: Area2D) -> void:
	...
	print("Cat 1 detector triggered by: ", detected)
...
func _on_chase_area_exited(area: Area2D):
	...
	if detected == current_target and is_chasing:
		print("Cat 1 lost chase target.")
		current_target = null
		is_chasing = false

func attack_target():
	if current_target and current_target.has_method("take_damage"):
		print("Cat 1 attacking: ", current_target)
